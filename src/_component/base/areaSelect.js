...
export default class AreaSelect extends Component{
    ...
    componentWillReceiveProps(newProps){
        if(newProps.value){
            ...
            let ar=newProps.value.address;
            
            let pr_id,cts,ct_id,ars,ar_id;
    ...
    provinceChange(e,value){
        ...
        if(index==-1){
            ...
            this.setState({
                ...
                city_id:-1,
                area_id:-1,
                cities:this.state.provinces[index].city,
                areas:[]
            });
        }
    }
    cityChange(e,value){
        ...
        if(index==-1){
            ...
            this.setState({
                city:this.state.cities[index].name,
                city_id:index,
                area_id:-1,
                areas:this.state.cities[index].area
            });
        }
    }
    areaChange(e,value){
    ...
    render(){
        let province_options=this.state.provinces.map((ele,index)=><MenuItem innerDivStyle={styles.menuItem} value={index} key={index} primaryText={ele.name} />);
        province_options.unshift(<MenuItem innerDivStyle={styles.menuItem} value={-1} key={-1} primaryText={"省份"} />);
        
        let city_options=[];
        if(this.state.cities.length)
            city_options=this.state.cities.map((ele,index)=><MenuItem innerDivStyle={styles.menuItem} value={index} key={index} primaryText={ele.name} />);
        city_options.unshift(<MenuItem innerDivStyle={styles.menuItem} value={-1} key={-1} primaryText={"市"} />);

        let area_options=[];
        if(this.state.areas.length)
            area_options=this.state.areas.map((ele,index)=><MenuItem innerDivStyle={styles.menuItem} value={index} key={index} primaryText={ele} />);
